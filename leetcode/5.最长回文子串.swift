...
class Solution {
    func longestPalindrome(_ s: String) -> String {
        ...
        let temp = Array(s)
        var dp = [[Bool]](repeating: [Bool](repeating: false, count: temp.count), count: temp.count)
        ...
        var l = 0, r = 0
        for right in 1 ..< temp.count {
            for left in 0 ..< right {
                if temp[left] == temp[right] {
                    if dp[left + 1][right - 1]  || right - left < 3{
                        dp[left][right] = true
                        if right - left > r - l {
                            l = left
                            r = right
